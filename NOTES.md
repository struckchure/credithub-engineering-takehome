...
**Guard tests, separate from contract tests.** `test_payments.py` pins what an
operator sees; `test_guards.py` pins the three mechanisms underneath — the float
tolerance, the partial unique index, and the lost-race fallback. Each was written
by deleting the guard and checking the test went red, because a test that passes
...
1. **Money must not be `Float`.** Store integer kobo or `Numeric(12, 2)`. The
   tolerance constant papers over the comparisons, but the storage is unchanged:
   `total_paid += amount` accumulates binary error on every payment, and
   `test_guards.py` carries two real Naira balances that already break without
   it — 462181.08 paid in two instalments sums a hair short, 254412.80 leaves a
   remainder that reads as an overpayment. Left as inherited schema, deliberately;
   this is the one I'd fix first.
2. **True concurrency still isn't in the suite.** All 26 tests are green, and
   `test_guards.py` covers the lost race deterministically by stubbing the
   in-code check so the database has to catch it. What is still hand-verified
   only is *actual* parallelism — threads racing one `external_ref` and one loan
   — which SQLite can't meaningfully exercise anyway.
3. **Removing the in-code duplicate check leaves the suite green**, because the
   unique index catches it on commit. That is defence in depth working as
   intended, not an untested path — but it does mean the fast-path check is not
   pinned by a test, only the guarantee is.
4. **`FOR UPDATE` is unverified here.** SQLite ignores it and serializes writers
   instead, so the concurrency guarantee is only real on Postgres and untested
   against it.
5. **The index needs a migration.** There is no Alembic setup; `create_all` only
   ...
   reconciled first.
6. **Webhook auth is a shared token in source.** `WEBHOOK_TOKEN` is a constant in
   ...
   config.
7. **No replay window or amount sanity bounds.** An event from six months ago,
   or an implausible amount, is accepted as readily as a fresh one.
8. **Audit actor is always `"webhook"`.** Good enough to trace a payment; not
   enough to answer "who authorised this" in an audit.
9. **The feed is unbounded.** `GET /payment-events` returns every event ever
   ...
   `received_at`.
10. **Timestamps go over the wire without a timezone.** The columns are plain
...
11. **The unknown-loan branch only works because SQLite doesn't enforce foreign
...
12. **`reason_code` has no migration.** New nullable column, and `create_all`
...
13. **Code and prose are two sources of truth.** `reason_code` and `reason` are
   only consistent because they are set in the same `_reject` call; a fifth branch
   that forgets one would fail no test.
14. **The panel aggregates client-side and polls every 5s**, which makes item 9
...
15. **`/audit-log` has no auth.** Neither does `/payment-events`. The trail leaks
   the operational picture of the whole book to anyone who can reach the port.
16. **"Unbooked value" is a triage number, not an accounting figure.** Duplicates
   are excluded because that money was booked once; unknown-loan amounts may
   already be settled at the rail.
17. **No frontend tests.** No runner is configured and I did not add one. The
...
18. **The test fixture wipes the working database.** `conftest.py` runs
